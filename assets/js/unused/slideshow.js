/* Slideshow JS for AWS Website | Based on Carrd.co | License: MIT */
...
(function() {
    ...
    function slideshowBackground(id, settings) {
        ...
        // Defer?
        if (this.defer) {
            // Add to scroll events if they exist
            if (typeof scrollEvents !== 'undefined') {
                scrollEvents.add({
                    element: this.$target,
                    enter: function() {
                        _this.preinit();
                ...
                // If scrollEvents doesn't exist, just preinit immediately
                this.preinit();
            }
        }
        // Otherwise ...
    ...
    // Make slideshowBackground available globally
    window.slideshowBackground = slideshowBackground;
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        function initSlideshows() {
            ...
            var slideshowElements = document.querySelectorAll('.productSlider.slideshow');
            
            // Loop through each slideshow element
            slideshowElements.forEach(function(element) {
                var id = element.id;
                if (!id) return;
                
                // Find the background element
                var bgElement = element.querySelector('.bg');
                if (!bgElement) return;
                
                // Find all image divs inside the background
                var imageDivs = bgElement.querySelectorAll('div[role="img"]');
                if (!imageDivs.length) return;
                
                // Create images array from the divs
                var images = [];
                imageDivs.forEach(function(div) {
                    // Extract image URL from inline style
                    var bgImage = div.style.backgroundImage;
                    var src = bgImage.replace(/^url\(['"](.+)['"]\)$/, '$1');
                    
                    // Get caption from aria-label
                    var caption = div.getAttribute('aria-label') || '';
