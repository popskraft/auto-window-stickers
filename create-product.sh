...
# create-product.sh - Script to create both content and data files for a new product
# Usage: ./create-product.sh "Product Name" [exterior|interior]

# Check if product name is provided
if [ -z "$1" ]; then
    echo "Error: Product name is required."
    echo "Usage: ./create-product.sh \"Product Name\" [exterior|interior]"
    exit 1
fi
...
if [ -f "$CONTENT_FILE" ]; then
    echo "Error: Content file already exists: $CONTENT_FILE"
    exit 1
fi
